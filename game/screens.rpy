...
style hotspot:
    activate_sound "audio/button_press.ogg"
...
style slider:
    ...
    left_bar Frame("gui/slider/bar_full.png",36,36)
    right_bar Frame("gui/slider/bar_null.png",36,36)
    thumb "gui/slider/htumb.png"
    ...
    if quick_menu:
        ...
        hbox:
            ...
            vpgrid:
                cols 5
                spacing 30
                imagebutton auto "gui/back_%s.png" action Rollback():
                    tooltip _("Назад")
                imagebutton auto "gui/history_%s.png" action ShowMenu('history'):
                    tooltip _("История")
                imagebutton auto "gui/skip_%s.png" action Skip(fast=False, confirm=True):
                    tooltip _("Пропуск")
                imagebutton auto "gui/interface_%s.png" action HideInterface():
                    tooltip _("Скрыть интерфейс")
                imagebutton auto "gui/menu_%s.png" action ShowMenu('game_menu'):
                    tooltip _("Меню")
...
screen main_menu():
    ...
    ## заменять этот.
    tag menu
    default tt = Tooltip(_("Наведите на обьект"))
    add "main_menu_par"


    imagemap:
        add "gui/main_menu_ground.png"
        add "images/main menu prlx/main_menu_C.png"
        add "images/main menu prlx/main_menu_B_2.png"
        add "gui/main_menu_tablica.png"
        idle "gui/main_menu_idle.png"
        hover "hovered_menu"
        alpha True

        if persistent.number_of_try != 0:
            hotspot (1001, 1114, 280, 256) action Continue():
                hovered tt.Action(_("Продолжить"))
            hotspot (1287, 1106, 290, 263) action Start("start"):
                hovered tt.Action(_("Начать игру"))
        else:
            hotspot (979, 1098, 612, 288) action Start("start"):
                hovered tt.Action(_("Начать игру"))

            
        hotspot (1845, 375, 713, 791) action ShowMenu("load"):
            hovered tt.Action(_("Загрузить"))

        hotspot (0, 210, 715, 999) action ShowMenu("preferences"):
            hovered tt.Action(_("Настройки"))
            
        hotspot (1090, 664, 388, 417) action Quit(confirm=True): 
            hovered tt.Action(_("Выход"))


    transform:
        rotate 6
        $ renpy.transition(Dissolve(0.18, alpha=True))
        text tt.value:
            xpos 800 
            ypos 275 
            size 40
    ...
    transform:
        ...
        text _("{i}забытых богов{/i}"):
            ...
            font gui.main_menu_font
    window:
        background None#Frame("gui/frame.png")
        xysize (250, 1000)
        xpos 0.5
        ypos 0.7
        add Crop((20,10,250,1000),"fire_main")
...
screen madness():
    vbox:
        ...
        xanchor 0.5
        yanchor 0.5
        vbar value stp_madness range 100:
            ysize 400
            xsize 100
            xalign 0.5
        text _("Рассудок") color "#fff"
...
screen game_menu():
    ...
    frame:
        ...
        vbox:
            ...
            textbutton _("Настройки") action ShowMenu("preferences")
            textbutton _("История") action ShowMenu("history")
            textbutton _("Главное меню") action MainMenu(confirm=True, save=True)


style menu_button_button is gui_button
style menu_button_button_text is gui_button_text

style menu_button_button:
    xalign 0.5
...
screen about():
    ...
    if main_menu:
        add gui.about_menu_background at blur(blur=30)
...
screen file_slots():
    ...
    if main_menu:
        add gui.loadSave_menu_background at blur(blur=30)
        ...
        fixed:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:    
                        ...
                        text ("%s."% slot +" "+ FileSaveName(slot)+" - "+FileTime(slot, format=_("%d %m %Y, %H:%M"), empty=_("Пустой слот"))):
            ...
            hbox:
                ...
                if config.has_quicksave:
                    textbutton _("{#quick_page}Быстр.сохр.") action FilePage("quick")
...
screen preferences():
    ...
    if main_menu:
        add gui.preferences_menu_background at blur(blur=30)
            ...
            vbox:
                ...
                hbox:
                    ...
                    vbox:
                        ...
                        if "text_speed" == 0:
                            $ text_speed = "Мгновенно"
                        else:
                            $ text_speed = int(preferences.text_cps)
                        label _("Скорость текста")
                        text _("[text_speed] сим/сек") style "label_text":
